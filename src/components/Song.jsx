...
const Song = () => {
  ...
  return (
    ...
    <div className='pt-[106px] max-lg:pt-20 max-md:pt-16 max-sm:pt-12 px-4 max-w-[1160px] mx-auto'>
      <h2 className='font-poppins text-custom-gray font-semibold lg:-mb-8 leading-[58.45px] max-lg:leading-[48px] max-md:leading-[40px] max-sm:leading-[32px] text-5xl max-lg:text-4xl max-md:text-3xl max-sm:text-2xl'>Songs</h2>
      <div className='max-w-[1140px] gap-[51px] pb-[45px] max-lg:pb-9 max-md:pb-8 max-sm:pb-3 max-lg:gap-12 max-md:gap-8 max-sm:gap-3 max-lg:flex-col max-lg:items-center mx-auto flex'>
        <table className='w-full lg:mt-[52px]'>
          ...
          <tbody>
            {TABLE_DATA_LIST.map(function (data, index) {
              return <tr key={index}>
                <td className='text-sm max-sm:text-xs leading-6 text-customGray pb-11 max-md:pb-8 max-sm:pb-5 max-lg:pr-1'>0{index + 1}</td>
                ...
                <td className='text-customLightGray text-xs max-sm:text-[10px] leading-6 pb-11 max-md:pb-8 max-sm:pb-5'>Nov 4, 2023</td>
                <td className='text-customLightGray text-xs max-sm:text-[10px] leading-6 text-right pb-11 max-md:pb-8 max-sm:pb-5'>{data.timesPlayed}</td>
              </tr>
            })}
          </tbody>
        </table>
